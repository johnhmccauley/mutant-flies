...
(function (root) {
  ...
  var EMPTY = 0, BRICK = 1, TREE = 2, MARBLE = 3,
      VAT_TAR = 4, VAT_WATER = 5, COOLED = 6;
  ...
  var NOTHING = 0, BOOTS = 1, FROST = 2, AXE = 3, JAR = 4;
  var ITEMS = {
    ...
    3: { key: "axe",   name: "Axe",   blurb: "Three swings. Trees do not move any other way.", colour: 0xb0b6bd },
  ...
  var BOOTS_TURNS = 70, FROST_TURNS = 50, AXE_SWINGS = 3;
  ...
  /* ==================================================================
     ...
     ================================================================== */
  var FEATURE_AT = { slopes: 4, boots: 5, frost: 6, trees: 7, axe: 7,
                     marbles: 8, jar: 9, tar: 10, water: 11 };
  ...
  var PLAN = [
    ...
    /* 13 */ { kinds: ["spider", "wasp"], PE: 135, PA: 125, adds: [] },
    /* 14 */ { kinds: ["fly", "spider", "beetle"], PE: 150, PA: 120, adds: [] },
    /* 15 */ { kinds: ["wasp", "beetle"], PE: 125, PA: 115, adds: [] },
  ...
  Game.prototype.solid = function (c, r) {
    ...
    if (v === BRICK || v === TREE || v === VAT_TAR || v === VAT_WATER || v === COOLED) return true;
  ...
  Game.prototype.makeFurniture = function () {
    ...
    if (f.axe) scatter(1 + rnd(2), function (c, r) { self.item[self.idx(c, r)] = AXE; });
          ...
          if (this.steps < cost2) ev.strained = true;
            ...
            if (run) {
              ...
              if (!this.inField(cc, rr)) {
                /* over the edge of the cellar: the original drew it outside
                   the graphics viewport, where it was clipped and lost */
              ...
              } else if (this.at(cc, rr) !== EMPTY) {
                /* a tree or a marble at the end of the run stops it dead */
                ev.lostOverEdge = 0; this.bricks--; ev.crunched = 1;
              } else {
  ...
  Game.prototype.take = function (it, ev) {
    ...
    else if (it === AXE) this.axe += AXE_SWINGS;
    else if (it === JAR) {
      /* nobody labelled it */
  ...
  Game.prototype.rollMarbles = function (ev) {
    for (var i = 0; i < this.marbles.length; i++) {
      ...
      for (var s = 0; s < steps && m.v > 0; s++) {
        ...
        if (what === TREE || what === MARBLE || what === COOLED) { this.bounce(m, 0.35); continue; }
  ...
  Game.prototype.blocksFluid = function (c, r, kind) {
    var v = this.at(c, r);
    if (v === -1) return true;
    if (v === BRICK || v === COOLED || v === VAT_TAR || v === VAT_WATER) return true;
  ...
  root.MutantFly = {
    ...
    VAT_TAR: VAT_TAR, VAT_WATER: VAT_WATER, COOLED: COOLED,
    DRY: DRY, WATER: WATER, TAR: TAR,
    NOTHING: NOTHING, BOOTS: BOOTS, FROST: FROST, AXE: AXE, JAR: JAR, ITEMS: ITEMS,
